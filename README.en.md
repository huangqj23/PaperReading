...
- **[2026-07-18]** 📄 Added optional MinerU PDF-to-Markdown extraction and structure-preserving Chinese translation for extracted papers.
...
| MinerU API | Converts a reachable PDF URL into structured Markdown with formulas, tables, and images |
| OpenAI-compatible translation API | Translates extracted Markdown into Chinese while preserving document structure, formulas, code blocks, links, and images |
...
### MinerU extraction and Chinese translation

These two scripts are optional and are not required by the default PyMuPDF path. They use `requests`:

```bash
python3 -m pip install requests
```

To extract a remotely accessible PDF with the MinerU v4 API, create a token at [MinerU API Management](https://mineru.net/apiManage), then run:

```bash
export DEEPPAPERNOTE_MINERU_TOKEN="<your-mineru-token>"
...
This produces `./paper/_mineru_full.md` and saves extracted images under `./paper/images/`. The MinerU API currently requires a reachable PDF URL; use the default PyMuPDF path for a local-only PDF.

To translate the extracted Markdown into Chinese through an OpenAI-compatible chat-completions API:

```bash
export DEEPPAPERNOTE_TRANSLATE_KEY="<your-api-key>"
export DEEPPAPERNOTE_TRANSLATE_BASE_URL="https://api.deepseek.com/v1" # optional
export DEEPPAPERNOTE_TRANSLATE_MODEL="deepseek-v4-pro"                # optional
...
The translation script preserves Markdown headings, formulas, code blocks, links, image references, and HTML tables. MinerU extraction sends the PDF URL to MinerU, and translation sends extracted text to the configured model provider; review the providers' privacy and billing policies before enabling them.

...
Engines:
  pymupdf    (default) PyMuPDF plain text extraction
  pymupdf_md PyMuPDF Markdown extraction (better structure, formulas, tables)
  mineru     MinerU v4 API extraction (best for academic papers, needs token)
"""
...
SUPPORTED_ENGINES = ("pymupdf", "pymupdf_md", "mineru")
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="PDF extraction engine: pymupdf (plain text), pymupdf_md (Markdown), mineru (MinerU v4 API).",
    )
    return p
...
def extract_page_texts_pymupdf_md(pdf_path: Path, max_pages: int | None) -> list[dict[str, Any]]:
    """PyMuPDF Markdown extraction — preserves structure, headings, tables, formulas."""
    if fitz is None:
        raise RuntimeError("PyMuPDF is required for source text extraction.")
    doc = fitz.open(pdf_path)
    try:
        page_limit = len(doc) if max_pages is None else min(len(doc), max_pages)
        return [
            {"page": page_index + 1, "text": doc[page_index].get_text("markdown")}
            for page_index in range(page_limit)
        ]
    finally:
        doc.close()
...
def extract_raw_sections_from_md(page_texts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Parse per-page Markdown into sections using #/##/### headings."""
    seen: dict[str, int] = {}
    sections: list[dict[str, Any]] = []
    current: dict[str, Any] | None = new_record("preamble", "preamble", 1, seen)
    ...
    for page in page_texts:
        ...
        for raw_line in str(page.get("text", "")).splitlines():
            ...
            if heading_match:
                ...
                current = new_record("section", title, page_number, seen)
                continue
            # Skip image references and HTML-only lines
            if re.match(r"^!\[.*\]\(.*\)$", line) or re.match(r"^\s*<[^>]+>\s*$", line):
                continue
            current.setdefault("_lines", []).append(line)
...
def _resolve_pdf_url(record: dict[str, Any]) -> str:
    url = normalize_whitespace(
        str(record.get("pdf_url") or record.get("source_url") or "")
    )
    if url and url.lower().endswith(".pdf"):
        return url
    arxiv_id = normalize_whitespace(str(record.get("arxiv_id", "")))
    if arxiv_id:
...
def _run_mineru_engine(
    record: dict[str, Any],
    raw_sections_output: str,
    full_text_output: str,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    from mineru_client import run as mineru_run, split_markdown_sections

    pdf_url = _resolve_pdf_url(record)
    if not pdf_url:
        raise SystemExit(
            "MinerU engine requires a PDF URL. Provide a paper with arxiv_id, "
            "pdf_url, or source_url ending in .pdf."
        )

    out_dir = str(
        Path(raw_sections_output or full_text_output or ".").expanduser().resolve().parent
    )
    mineru_manifest = mineru_run(pdf_url, out_dir)

    sections = mineru_manifest.get("sections", [])
    if not sections:
        markdown_path = mineru_manifest.get("markdown_path", "")
        if markdown_path:
            md_text = Path(markdown_path).read_text(encoding="utf-8")
            sections = split_markdown_sections(md_text)

    page_texts: list[dict[str, Any]] = [{"page": 0, "text": ""}]
...
def main() -> None:
    ...
    if args.engine == "mineru":
        sections, page_texts = _run_mineru_engine(
            record, raw_sections_output, args.full_text_output
        )
        pdf_path = resolve_pdf_path(record) or Path("mineru_api")
        manifest = {
            "status": "ok",
            "script": "extract_source_text.py",
            "schema_version": 1,
            "paper_id": record.get("paper_id") or paper_id_for_record(record),
            "title": record.get("title", ""),
            "source_kind": "mineru_api",
            "raw_sections_path": (
                str(Path(raw_sections_output).expanduser().resolve()) if raw_sections_output else ""
            ),
            "full_text_md_path": (
                str(Path(args.full_text_output).expanduser().resolve())
            ...
            "sections": [
                {
                    key: section.get(key)
                    for key in (
                        "section_id", "kind", "title",
                        "page_start", "page_end", "char_count", "text_hash_sha256",
                    )
                }
                for section in sections
            ],
            ...
            "captions": caption_manifest(page_texts, sections),
            "math_index": [],
            "language_hint": language_hint_for_text(
        ...
        if isinstance(record.get("source_manifestation"), dict):
            manifest["source_manifestation"] = record.get("source_manifestation")
    elif args.engine == "pymupdf_md":
        pdf_path = resolve_pdf_path(record)
        if pdf_path is None:
            raise SystemExit("extract_source_text.py requires a resolvable local PDF path.")
        page_texts = extract_page_texts_pymupdf_md(pdf_path, args.max_pages)
        sections = extract_raw_sections_from_md(page_texts)
        ...
        manifest["source_kind"] = "pdf_markdown"
    else:
        pdf_path = resolve_pdf_path(record)
        if pdf_path is None:
            raise SystemExit("extract_source_text.py requires a resolvable local PDF path.")
